...
mbar,E_kln,E_expect,dE_expect = get_mbar_expectation(replica_energies,temperature_list,len(temperature_list))

mbar,E_kln,DeltaE_expect,dDeltaE_expect = get_mbar_expectation(E_kln,temperature_list,len(temperature_list),output='differences')

mbar,E_kln,E2_expect,dE2_expect = get_mbar_expectation(E_kln**2,temperature_list,len(temperature_list))

df_ij,ddf_ij = get_free_energy_differences(mbar)

C_v,dC_v = calculate_heat_capacity(E_expect,E2_expect,temperature_list)

file_name = "heat_capacity.png"
figure = pyplot.figure(1)
temperatures = np.array([temperature for temperature in Temp_k])
C_v = np.array([allCv_expect[i][0][0] for i in range(len(allCv_expect[:]))])
print(C_v)
print(temperatures)
#C_v = np.array([allCv_expect[0][i][0] for i in range(len(allCV_expect[0]))])
pyplot.plot(temperatures,C_v,figure=figure)

pyplot.xlabel("Temperature ( Kelvin )")
pyplot.ylabel("C$_v$ ( kcal/mol * Kelvin )")
pyplot.title("Heat capacity")
