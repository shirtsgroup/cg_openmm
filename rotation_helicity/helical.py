...
print("Check the directors the same?:", np.max(up-up2,axis=0)) # this should be nearly zero, just a check.
...
def root_for_cpk(x,up,z):  # might have broken at some point? Not currently using.
    ...
    p2 =  2*sinv/p*np.sum(-sinv*cosv*uz + (cosv*cosv*cospz - p*p*sinpz)*uy + (cosv*cosv*sinpz-p*p*cospz)*ux )
...
results = scipy.optimize.minimize(obj_for_cpv,[0,trialp,trialk],args=(up,z),method='nelder-mead')
phase = results.x[0]
