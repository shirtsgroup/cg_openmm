...
for epsilon in epsilon_list:
    ...
    n_unfolded = len(
        nonnative_ensemble_energies
    )  # ratio of number of unfolded states to folded states. We keep number of folded states at 1
...
toplot = [A_list, Pf_list, E_list, S_list, C_list]
titles = [
...
legend = [str(round(epsilon._value, 2)) for epsilon in epsilon_list]
legend_title = r"$\epsilon$ (kcal/mol)"

for p, t, y, name in zip(toplot, titles, ylabels, file_names):
    figure = plt.figure(1)
    for index in range(len(p)):
        (line,) = plt.plot(toplotT, p[index])
        line.set_label(legend[index])
    plt.title(t)
    plt.xlabel("T (Kelvin)")
    plt.ylabel(y)
