...
for cutoff in cutoff_list:
    ...
    for pose in random_poses:
        Q = fraction_native_contacts(
            ...
            pose,
            native_structure,
            native_structure_contact_distance_cutoff=cutoff,
...
# Plot the fraction of native contacts vs. the cutoff distance for defining a native contact
x = np.array([float(cutoff.__div__(cgmodel.get_sigma(0))) for cutoff in cutoff_list])
y = np.array(
    [float(fraction_native_contacts) for fraction_native_contacts in list_fraction_native_contacts]
)
