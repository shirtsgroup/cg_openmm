...
# Global definitions
polymer_length_list = [10,20,30,40,50]
backbone_lengths=[1]
sidechain_lengths=[1]
...
epsilon_list = [ unit.Quantity(1.0,unit.kilocalorie_per_mole) ]
#epsilon_list = [ unit.Quantity((0.9 + i*0.05),unit.kilocalorie_per_mole) for i in range(grid_size)]
for polymer_length in polymer_length_list:
#for sigma in sigma_list: 
 for epsilon in epsilon_list:
  print("Performing simulations and heat capacity analysis for a coarse grained model")
  print("with a length of "+str(polymer_length)+" and epsilon values of "+str(epsilon))
  sigma = 1.8 * bond_length
  ...
  output_data = str(str(top_directory)+"/length_"+str(polymer_length)+"_eps_"+str(epsilon._value)+".nc")
...
legend_labels = [length for length in polymer_length_list]
...
pyplot.title("Heat capacity for variable polymer lengths")
