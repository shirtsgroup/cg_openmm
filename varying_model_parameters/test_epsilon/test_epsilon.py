...
epsilon_list = [
    unit.Quantity((0.25 + i * 0.25), unit.kilocalorie_per_mole) for i in range(grid_size)
]
for epsilon in epsilon_list:
    ...
    C_v, dC_v, new_temp_list = get_heat_capacity(
    ...
    for temp in range(len(new_temp_list)):
        file.write(
            str(round(epsilon._value, 2))
            + " "
            + str(round(new_temp_list[temp], 2))
            + " "
            + str(round(C_v[temp], 4))
            + "\n"
        )
    file.close
