...
sigma_list = [ (2.6 + i*0.05) * bond_length for i in range(grid_size)]
epsilon_list = [ unit.Quantity(2.8,unit.kilocalorie_per_mole) ]
...
sigmas = [ sigma._value for sigma in sigma_list]
file_name = "folding_T.png"
figure = pyplot.figure(2)
pyplot.plot(sigmas,folding_T)
pyplot.xlabel("$\sigma / r_{bond}$")
pyplot.ylabel("Folding T (Kelvin)")
pyplot.title("Folding T for variable $\sigma / r_{bond}$")
pyplot.savefig(file_name)
pyplot.show()
pyplot.close()
