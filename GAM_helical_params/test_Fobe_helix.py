...
for sigma in sigma_list:
 for epsilon in epsilon_list:
  ...
  pitch_list = []
  monomers_per_turn_list = []
  for pose in minimum_energy_structures:
    cgmodel.positions = pose
    radius,pitch,monomers_per_turn = get_helical_data(cgmodel)
    radius_list.append(radius)
    pitch_list.append(pitch)
    monomers_per_turn_list.append(monomers_per_turn)
  print(monomers_per_turn_list)
  print(radius_list)
  print(pitch_list)
  exit()
  monomers_per_turn = mean(np.array([float(mpt) for mpt in monomers_per_turn_list]))
  radius = mean(np.array([float(radius) for radius in radius_list]))
  pitch = mean(np.array([float(pitch) for pitch in pitch_list]))
