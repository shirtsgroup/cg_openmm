...
def optimize_lj(cgmodel, base_epsilon=0.0, sigma_attempts=3, epsilon_attempts=3):
    ...
    sigma_list = [
        (base_sigma).__add__(i * base_sigma.unit) for i in [j * 0.2 for j in range(-2, 3, 1)]
    ]
    epsilon_list = [
        (base_epsilon).__add__(i * base_epsilon.unit) for i in [j * 0.2 for j in range(-1, 3, 1)]
    ]
    ...
    for sigma_index in range(len(sigma_list)):
        for epsilon_index in range(len(epsilon_list)):
            ...
            replica_energies, replica_temperatures = replica_exchange(
