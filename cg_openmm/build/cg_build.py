            ...
            if backbone_bead in sidechain_positions:
                for sidechain in range(monomer_type['sidechain_length']):
                    ...
                    if particle_symbol not in elem.Element._elements_by_symbol:
                        mass = cgmodel.get_particle_mass(cg_particle_index - 1)
                        elem.Element(
                            element_index, particle_name, particle_symbol, mass)
                        particle_list.append(particle_symbol)
                        element_index = element_index + 1
                    cg_particle_index = cg_particle_index + 1
...
def write_xml_file(cgmodel, xml_file_name):
    ...
    xml_object.write(
        "  <Source> https://github.com/shirtsgroup/cg_openmm </Source>\n")
    ...
    for bond in cgmodel.bond_list:
        if all(bond[i] < len(unique_particle_names) for i in range(2)):
            ...
            xml_object.write(
                ...
                str(particle_2_name) +
                '"/>\n')
    xml_object.write('   <ExternalBond atomName="' +
                     str(unique_particle_names[0]) + '"/>\n')
    external_parent = unique_particle_names[len(
        unique_particle_names) - cgmodel.monomer_types[0]['sidechain_length'] - 1]
    xml_object.write(
    ...
    if cgmodel.include_nonbonded_forces:
        ...
        for particle_index in range(len(unique_particle_names)):
            ...
            sigma = cgmodel.get_sigma(
                particle_index).in_units_of(unit.nanometer)._value
...
def verify_topology(cgmodel):
    ...
    if cgmodel.num_beads != cgmodel.topology.getNumAtoms():
        ...
        print("There are " + str(cgmodel.num_beads) +
              " particles in the coarse grained model\n")
        print("and " + str(cgmodel.topology.getNumAtoms()) +
              " particles in the OpenMM topology.")
        exit()
    ...
    if cgmodel.polymer_length != cgmodel.topology.getNumResidues():
        ...
        print("There are " + str(cgmodel.polymer_length) +
              " monomers in the coarse grained model\n")
        print("and " + str(cgmodel.topology.getNumResidues()) +
              " monomers in the OpenMM topology.")
        exit()
            ...
            if backbone_bead in [monomer_type['sidechain_positions']]:
                for sidechain_bead in range(monomer_type['sidechain_length']):
                    particle_symbol = cgmodel.get_particle_name(
                        cg_particle_index)
                    element = elem.Element.getBySymbol(particle_symbol)
                    particle = topology.addAtom(
                        particle_symbol, element, residue)
                    if sidechain_bead == 0:
                        if cgmodel.include_bond_forces or cgmodel.constrain_bonds:
                            topology.addBond(particle, last_backbone_particle)
                    if sidechain_bead != 0:
...
def verify_system(cgmodel):
    ...
    if get_num_forces(cgmodel) != cgmodel.system.getNumForces():
        ...
        print(" There are " + str(get_num_forces(cgmodel)) +
              " forces in the coarse grained model\n")
        print("and " + str(cgmodel.system.getNumForces()) +
              " forces in the OpenMM System().")
        exit()
    ...
    if cgmodel.num_beads != cgmodel.system.getNumParticles():
        ...
        print("There are " + str(cgmodel.num_beads) +
              " particles in the coarse grained model\n")
        print("and " + str(cgmodel.ssytem.getNumParticles()) +
              " particles in the OpenMM system.")
        exit()
    ...
    if cgmodel.constrain_bonds:
        if len(cgmodel.bond_list) != cgmodel.system.getNumConstraints():
            ...
            print("There are " + str(cgmodel.bond_list) +
                  " bond constraints in the coarse grained model\n")
            print("and " + str(cgmodel.system.getNumConstraints()) +
...
def test_force(cgmodel, force, force_type=None):
    ...
    if force_type == "Nonbonded":
        if cgmodel.num_beads != force.getNumParticles():
            print(
                "ERROR: The number of particles in the coarse grained model is different")
            print(
                "from the number of particles with nonbonded force definitions in the OpenMM NonbondedForce.\n")
            print("There are " + str(cgmodel.num_beads) +
                  " particles in the coarse grained model")
            print("and " + str(force.getNumParticles()) +
                  " particles in the OpenMM NonbondedForce.")
            success = False
        ...
        for nonbonded_interaction in cgmodel.nonbonded_interaction_list:
            ...
            int_energy = lj_v(
                particle_1_positions,
                particle_2_positions,
                sigma,
                epsilon)
        ...
        cgmodel.simulation = build_mm_simulation(
            ...
            unit.femtosecond,
            print_frequency=1)
        potential_energy = cgmodel.simulation.context.getState(
...
def add_rosetta_exception_parameters(
        cgmodel,
        nonbonded_force,
        particle_index_1,
        particle_index_2):
    ...
    for exception in range(nonbonded_force.getNumExceptions()):
        ...
        if [index_1, index_2] not in exception_list and [
                index_2, index_1] not in exception_list:
    ...
    if [particle_index_1, particle_index_2] not in exception_list and [
        ...
        sigma_1 = cgmodel.get_sigma(
            particle_index_1).in_units_of(unit.nanometer)
        epsilon_1 = cgmodel.get_epsilon(
            particle_index_1).in_units_of(unit.kilojoule_per_mole)
        charge_2 = cgmodel.get_particle_charge(particle_index_2)
        sigma_2 = cgmodel.get_sigma(
            particle_index_2).in_units_of(unit.nanometer)
        epsilon_2 = cgmodel.get_epsilon(
            particle_index_2).in_units_of(unit.kilojoule_per_mole)
        sigma = (sigma_1.__add__(sigma_2)) / 2.0
        epsilon = 0.2 * unit.sqrt(epsilon_1.__mul__(epsilon_2))
        nonbonded_force.addException(
            particle_index_1,
            particle_index_2,
            0.2 * charge_1 * charge_2,
            sigma,
            epsilon)
