...
print_frequency = 10 # Number of steps to skip when printing outputr
number_repeats = 5
total_simulation_time = 1000 * unit.picosecond
...
#sigmas = {'bb_bb_sigma': 0.5 * unit.angstrom, 'bb_sc_sigma': 0.5 * unit.angstrom,'sc_sc_sigma': 0.5 * unit.angstrom}
...
#epsilons = {'bb_bb_eps': 0.0 * unit.kilocalorie_per_mole,'bb_sc_eps': 0.0 * unit.kilocalorie_per_mole,'sc_sc_eps': 0.0 * unit.kilocalorie_per_mole}
...
bond_angle_force_constant = 120 * unit.kilojoule_per_mole / unit.radian / unit.radian
...
for polymer_length in [10,20,30]:
#for polymer_length in [40,60]:
...
# cg_model = CGModel(polymer_length=polymer_length,backbone_lengths=backbone_lengths,sidechain_lengths=sidechain_lengths,sidechain_positions=sidechain_positions,masses=masses,sigmas=sigmas,epsilons=epsilons,bond_lengths=bond_lengths,bond_force_constants=bond_force_constants,bond_angle_force_constants=bond_angle_force_constants,torsion_force_constants=torsion_force_constants,equil_bond_angles=equil_bond_angles,equil_torsion_angles=equil_torsion_angles,include_nonbonded_forces=True,include_bond_forces=True,include_bond_angle_forces = True,include_torsion_forces=True,constrain_bonds=constrain_bonds)
 ...
 integrator = LangevinIntegrator(temperature._value,
                                 0.1,
                                 simulation_time_step.in_units_of(unit.picosecond)._value)


 simulation = build_mm_simulation(cg_model.topology, cg_model.system, positions, temperature=temperature,simulation_time_step=simulation_time_step,print_frequency=print_frequency)
 simulation.context.setPositions(cg_model.positions)

 simulation.minimizeEnergy(maxIterations=1000)
 times = []
 print("About to run scaling tests")
 nsteps = int(total_simulation_time.in_units_of(unit.picosecond)._value / simulation_time_step.in_units_of(unit.picosecond)._value)
 ...
 # Run simulation using CPU
  ...
  times.append(time)
 time = mean(times)
 stdtime = np.std(np.array(times))
 #print(time)
 file = open(str(str(top_directory)+"/time.dat"),"a")  
 # compute conversion to NS/hour.
 # We get the time in seconds, and the simulation length is in ps.  Convert simulation to 1M steps/hr
 timeconvert = nsteps*(3600/1000000)
 file.write(str(polymer_length)+" "+str(timeconvert/time)+" "+str(timeconvert*stdtime/(time*time))+"\n") # use error propagation
 file.close()
exit()
